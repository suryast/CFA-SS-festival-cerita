...
module.exports = function () {
	...
	// Direct url link to a specific volunteer
	_helpers.volunteerUrl = function (postSlug, options) {
	...
	* */
	...
	_helpers.paginationVolunteerNavigation = function (pages, currentPage, totalPages, options) {
		var html = '';
		...
		_.each(pages, function (page, ctr) {
			...
			// get the pageUrl using the integer value
			var volunteerUrl = _helpers.volunteerUrl(page);
			// wrapup the html
			html += '<li' + liClass + '>' + linkTemplate({ url: volunteerUrl, text: pageText }) + '</li>\n';
		});
		return html;
	};
